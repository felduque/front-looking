...
export default function CardDetail() {
  ...
  const handleUpdateComment = (event) => {
    ...
    axios
      .patch(
        `https://looking.fly.dev/comment/edit/${editComment.commentId}`,
        editCommento
      )
      .then((response) => {
  ...
  const handleSubmit = (event) => {
    ...
    fetch("https://looking.fly.dev/comment/createcomment", {
      method: "POST",
  ...
  function deleteComment(id) {
    fetch(`https://looking.fly.dev/comment/delete/${id}`, { method: "DELETE" })
      .then((response) => {
        if (response.ok) {
